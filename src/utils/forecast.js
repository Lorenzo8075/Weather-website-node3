...
const forecast = (latitude, longitude, callback) => {
    ...
    request({ url: url, json: true }, (error, response) => { //request({ url, json: true }, (error, {body}}) =>
        if (error) {
            ...
            callback(undefined, "The local date and time is " + response.body.location.localtime + ". It's " + response.body.current.weather_descriptions[0] +
                " at " + response.body.current.temperature + " degress. But feels like " +
